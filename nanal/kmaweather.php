	...
	function area_weather_xml_parser($x,$y)
	{
		...
		else
		{
			...
			while (!feof($fp2))
			{
				...
				// 총 11개:시간, 날, 온도, 24최고온도, 24최저온도, 강수상태, 날씨한글, 날씨영어, 강수확률, 풍속, 습도
				...
				if(ereg("<tmx>",$line))$array[$i]['tmx']= $this->iconv_UTF_8(trim(strip_tags($line))); //최고 온도
				if(ereg("<tmn>",$line))$array[$i]['tmn']= $this->iconv_UTF_8(trim(strip_tags($line))); //최저 온도
				...
				if(ereg("<ws>",$line))$array[$i]['ws']= $this->iconv_UTF_8(trim(strip_tags($line))); //풍속
				if(ereg("<reh>",$line))$array[$i]['reh']= $this->iconv_UTF_8(trim(strip_tags($line))); //습도%
...
echo print_r($obj->area_weather_xml_parser('69','106'));
*/
?>